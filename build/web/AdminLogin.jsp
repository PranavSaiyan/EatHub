<%-- 
    Document   : AdminLogin
    Created on : Dec 14, 2017, 11:40:00 AM
    Author     : user
--%>
  ...
  <script>
      function encryptemailpass(){
        ...
        if(mail=='' || pass==''){
             $.alert({    
                    title: 'Alert!',
                    content: 'Please fill up your credentials',
                    });
        }        
        else{
...
<body>
  ...
  <%  if(request.getParameter("error")!=null){ %>
            <div class="alert alert-info" style=" font-family: 'Bree Serif', serif;text-align: center;font-size: 40px;">
                <strong><% if(Integer.parseInt(request.getParameter("error"))==1){ out.print("Wrong Credentials</strong></div>");}
                else{out.print("Account does'nt exists</strong></div>");}
                   } %>
  <div class="login">
      <h1> <img  src="images/Photo_1507406926754.png" width="100%"/></h1>
      <form >
      ...
      <form id="loginadmin" method="post" action="AdminSignin">
          <input type="hidden" id="hashcode" name="hashcode"/>
          <button type="button" class="btn btn-primary btn-block btn-large" onclick="encryptemailpass()">Let me in.</button>
      </form>
</div>
